...
(function (global, factory) {
   ...
   * Object-based format.
   */
  function normalizeProps$1 (options) {
  ...
  function deactivateChildComponent (vm, direct) {
    ...
    if (!vm._inactive) {
      ...
      callHook$1(vm, 'deactivated', 'onLaunch',
  'onLoad',
  'onShow',
  'onReady',
  'onHide',
  ...
  Watcher.prototype.teardown = function teardown () {
    ...
    if (this.active) {
      ...
      var i = this.deps.length;
      while (i--) {
        this$1$1.deps[i].removeSub(this$1$1);
      }
      this.active = false;
    }
  };
  ...
  function createFunctionalComponent (
    ...
    var h = function (a, b, c, d) { return createElement$1(_context, a, b, c, d, true); };
  ...
  var KeepAlive = {
    ...
    destroyed: function destroyed () {
      ...
      for (var key in this$1$1.cache) {
        pruneCacheEntry(this$1$1.cache[key]);
      }
    },
  ...
  function callHook (vm, hook, params) {
    ...
    if (vm.$children.length) {
      vm.$children.forEach(function (v) { return callHook(v, hook, params); });
    }
      ...
      global.Page({
        ...
        onShow: function onShow () {
          ...
          this.rootVueVM.$nextTick(function () {
            this$1$1.rootVueVM._initDataToMP();
          });
        },
  ...
  function compareAndSetDeepData (key, newData, vm, data) {
    ...
    try {
      ...
      warn('Failed to diff key "' + key + '": ' + (e && e.message));
    }
  }
  ...
  function minifyDeepData (rootKey, originKey, vmData, data, _mpValueSet, vm) {
    try {
      if (vmData instanceof Array) {
        ...
        // Object
        var _keyPathOnThis = {}; // Collect this level's keyPath entries.
      ...
      warn('Failed to minify data at "' + rootKey + '.' + originKey + '": ' + (e && e.message));
    }
  }

  function getRootKey (vm, rootKey) {
    if (!vm.$parent.$attrs) {
      rootKey = '$root.0' + KEY_SEP$2 + rootKey;
      return rootKey
    } else {
      rootKey = vm.$parent.$attrs.mpcomid + KEY_SEP$2 + rootKey;
  ...
  function formatVmData (vm) {
    var $p = getParentComKey(vm).join(KEY_SEP$1);
    var $k = $p + ($p ? KEY_SEP$1 : '') + getComKey(vm);

    // getVmData 这儿获取当前组件内的所有数据，包含 props、computed 的数据
    // 改动 vue.runtime 所获的的核心能力
    var data = Object.assign(getVmData(vm), { $k: $k, $kk: ("" + $k + KEY_SEP$1), $p: $p });
    var key = '$root.' + $k;
    var res = {};
    res[key] = data;
    return res
  ...
  function handleProxyWithVue (e) {
    ...
    var vm = getVM(rootVueVM, comkey.split(KEY_SEP));
  ...
  Vue.prototype.$mount = function (el, hydrating) {
    ...
    if (options && (options.render || options.mpType)) {
      ...
      return this._initMP(mpType, function () {
        return mountComponent(this$1$1, undefined, undefined)
      })
    } else {
